...
public class AjustesUsuario extends javax.swing.JPanel {
    ...
    public AjustesUsuario(Clientes cliente) {
        initComponents();
        this.cliente = cliente;
        quitarLabels();
        txtNombre.setText(cliente.getNombre());
        txtApellidoP.setText(cliente.getApellido_Paterno());
        txtApellidoM.setText(cliente.getApellido_Materno());
        String fecha[] = cliente.getFecha().toString().split("-");
        jcYear.addItem(fecha[0]);
        jcDia.addItem(fecha[2]);
        Calendario mes = null;
        ponerMes(mes, fecha[1]);
        txtColonia.setText(cliente.getColonia());
        String calleNo[] = cliente.getCalle().split("#");
    ...
    private void initComponents() {
        ...
        add(jLabel1);
        jLabel1.setBounds(0, -140, 1370, 770);
    ...
    private void btnAgregarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAgregarActionPerformed
        ...
        if(correcto){
            ...
            if(txtCorreo.getText().equals(cliente.getEmail())){
                error = RestauranteService.actualizarClienteService(cliente, false);
            }else{
                cliente.setEmail(txtCorreo.getText());
                error = RestauranteService.actualizarClienteService(cliente, true);
            }
            switch(error){
                case 0:
